...
class MusicalScale {
  ...
  _loadScale (params) {
    ...
    for (let s = 0; s < this._scale.steps.length; s++) {
      ...
      let relOctave = offset + step > keys.length - 1 ? 1 : 0
      // generate the relative triads
      let triad = this._genTriad(s, idx, relOctave, this._scale.triads[s])
      // define the note
      let note = {
        step: s,
        note: keys[idx],
  ...
  _genTriad (s, offset, octave, t) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      // define the note
      chord.notes.push({ note: keys[idx], relOctave: relOctave })
    }
    return chord
  }
